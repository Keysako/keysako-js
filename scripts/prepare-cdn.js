...
if (!allFilesExist) {
  console.error(
    'Certains fichiers sources n\'existent pas. Assurez-vous de compiler le package core avec "npm run build" dans le répertoire packages/core.'
  );
  process.exit(1);
}
  ...
  if (!(await fs.promises.stat(templatePath).catch(() => false))) {
    ...
    if (await fs.promises.stat(currentFilePath).catch(() => false)) {
      ...
      templateContent = templateContent.replace(
        /https:\/\/cdn\.keysako\.com\/v\d+\/keysako-connect\.min\.js/g,
        'https://cdn.keysako.com/v{{ MAJOR_VERSION }}/keysako-connect.min.js'
      );
      templateContent = templateContent.replace(
        /<script type="module" src="https:\/\/cdn\.keysako\.com\/v\d+\/keysako-connect\.min\.js".*?><\/script>/g,
