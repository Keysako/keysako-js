...
describe('TokenManager', () => {
  ...
  beforeEach(() => {
    ...
    tokenManager = TokenManager.getInstance({ storageStrategy: 'localStorage' });
  });
  ...
  describe('saveTokens', () => {
    it('should store tokens in localStorage', async () => {
      ...
      await tokenManager.saveTokens(tokenResponse);
      ...
      if (storedTokens) {
        ...
        expect(parsedTokens.expires_at).toBeLessThanOrEqual(
          Date.now() + tokenResponse.expires_in * 1000 + 100
        );
      }
    });
  });
  ...
  describe('getAccessToken', () => {
    it('should return the stored access token', async () => {
      ...
      expect(result).toBe(tokenResponse.access_token);
    });
  ...
  describe('getIdToken', () => {
    it('should return the stored ID token', async () => {
      const tokenResponse = {
        access_token: 'access-token-123',
        id_token: 'id-token-456',
        expires_in: 3600,
      ...
      const result = await tokenManager.getIdToken();
      expect(result).toBe(tokenResponse.id_token);
    });
  ...
  describe('hasValidAccessToken', () => {
    it('should return true if token exists and is not expired', async () => {
      ...
      const result = await tokenManager.hasValidAccessToken();
  ...
  describe('clearTokens', () => {
    it('should remove all tokens from localStorage', async () => {
      const tokenResponse = {
        access_token: 'access-token-123',
        id_token: 'id-token-456',
        expires_in: 3600,
